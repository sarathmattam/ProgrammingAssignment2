...
## the inverse value from cache during the next iterations, provided there is change in the input matrix
## If a different matrix is provided as input, the inverse has to be recalculated and the cache need to be updated
...
cacheSolve <- function(x) {
  ...
  makeCacheMatrix()
  # Below function identifies whether the matrix provided in the previous iteration is the same as the 
  # one provided in the current iteration. If not, the cache need to be refreshed and inverse has to be 
  # computed again and the new inverse has be to cached
  vFlag <- setM(x)
  print(vFlag)
